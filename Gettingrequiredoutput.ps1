########################################################################################
    ...
    #Fetching server details from input file
    $serverList = Get-Content -Path "$inputFolderPath\serverList.txt"
...
$logfreespace = "DBCC SQLPERF('logspace')"

# Assuming $connectionDetails is defined somewhere in your script
$connection = New-Object System.Data.OleDb.OleDbConnection $connectionDetails

$command35 = New-Object System.Data.OleDb.OleDbCommand $logfreespace , $connection
$connection.Open()
...
$dataTable = $dataSet35.Tables[0]
$htmlTable = $dataTable | Select-Object * -ExcludeProperty RowError, RowState, Table, ItemArray, HasErrors, Status | ConvertTo-HTML -AS Table -Fragment -PreContent '<h2> LogFreeSpace</h2>'|Out-String



##Fetching drive space
    ...
    $test = ConvertTo-HTML -head $a -PostContent $frag1,$frag33,$frag5,$frag19,$htmlTable -PreContent "<h1 align='Center'>SQL Server Heatlh Check Report</h1>" #| Out-File $OutputFile_new
...
} -ArgumentList $serverList1

$result | Out-File "C:\Users\ANKUSHKU\Desktop\htmlreport.html"
